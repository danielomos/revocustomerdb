...
import africa.revoenergy.config.KafkaSseConsumer;
import africa.revoenergy.config.KafkaSseProducer;
...
class CustomerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/customer-kafka/publish?message=value-produce")).andExpect(status().isOk());
